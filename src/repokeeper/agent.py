...
def create_pr(
    ...
    tone = profile.get("response", {}).get("tone", "friendly")
    closing = profile.get("tone", {}).get("closing", "happy to help")
...
def run_agent(
    ...
    if skip_kw:
        print(f"[repokeeper] Skipping: matched skip keyword '{skip_kw}'")
        post_comment(
            issue_obj,
    ...
    try:
        ...
        # Call LLM
        print(f"[repokeeper] Calling LLM ({profile.get('agent', {}).get('model', 'deepseek-chat')})...")
